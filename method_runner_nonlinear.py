...
# from nonlinear import NotearsMLP, notears_nonlinear
import torch
...
from Topo_utils import create_Z,create_new_topo,find_idx_set,threshold_W,find_topo,find_idx_set_updated
import utils
# from nonlinear import notears_nonlinear,NotearsMLP
...
class TopoMLP(nn.Module):
    ...
    #         output.append(self._forward_i(x, ii))
    #
    #     return torch.cat(output, dim=1)
    def freeze_grad_by_index(self):
        pass
    ...
    #
    # def update_grad(self, X_torch, lambda1, lambda2):
    #     self.zero_grad()
    #     X_hat = self.forward(X_torch)
    #     loss = F.mse_loss(X_hat, X_torch)  # Assuming squared_loss is MSE
    #     l2_reg = 0.5 * lambda2 * self.l2_reg()
    #     l1_reg = lambda1 * self.layer0_l1_reg()
...
class TOPO_Nonlinear:
    ...
    def train(self, model):
        ...
        for i in range(0,int(self.num_iter)+1):
            ...
            loss = self.train_iter(model=model, optimizer=optimizer)
            # print(f"in training, iteration {i}, loss:{loss}")
            if self.lr_decay and self.opti == 'Adam' and (i+1)%500 == 0:
                # print(f"in training, iteration {i}, loss:{loss}")
                scheduler.step()
            elif self.lr_decay and self.opti == 'LBFGS' and (i+1)%10 == 0:
                scheduler.step()
            if self.opti == 'Adam' and (i+1)%200 ==0:
                # print(f"current W :\n{model.layer0_to_adj()}\n")
                if abs((loss - loss_progress[-1])/loss_progress[-1])<self.tol:
                    print(f"we are at iteration {i}, current loss {loss} and we break")
                    break
                else:
                    loss_progress.append(loss)
            if self.opti == 'LBFGS' and (i+1)%10 ==0:
                loss_progress.append(loss)
                if abs((loss - loss_progress[-1])/loss_progress[-1])<self.tol:
                    break
                else:
    ...
    #
    #     # Iterate over parameters of both models and copy the `requires_grad` setting
    ...
    def copy_model(model, model_clone):
        ...
        # Debugging: Confirm copying process
        # print("Copying model parameters and requires_grad settings...")
        for param_target, (name_source, param_source) in zip(model_clone.parameters(), model.named_parameters()):
            # print(
            #     f"Copying {name_source}: requires_grad from {param_source.requires_grad} to {param_target.requires_grad}")
            param_target.requires_grad = param_source.requires_grad
    ...
    def fit(self,
        ...
        while bool(idx_set):
            ...
            for i in range(idx_len):
                ...
                print('-----------------start to train model_clone-----------------')
                print(f"we are trying to work with topo: {topo_clone} ")
                # print(f"corresponding matrix should look like \n{~create_Z(topo_clone)+0}")
                loss_clone = self.train(model = model_clone)
                ###############################
                criterion_clone = utils.best_shd(W = model_clone.layer0_to_adj() ,W_true= self.B_true)
                print(f"min shd {criterion_clone[0]}, best thresholds:{criterion_clone[1]}, shd thres = 0.3: {criterion_clone[2]}")
                ###############################
                ...
                if loss_clone<self.loss:
                    ...
                    # self.model.load_state_dict(model_clone.state_dict())
                    ...
                    self.h, self.G_h = self.model._h(W=self.W_adj)
                    print(f"---good news---")
                    print(f"find better loss {self.loss} current topo {self.topo}")
                    criterion = utils.best_shd(W = model_clone.layer0_to_adj() ,W_true= self.B_true)
                    print(f"min shd: {criterion[0]}, best thresholds:{criterion[1]}, shd thres = 0.3: {criterion[2]}")
            ...
            if not indicator_improve:
                if large_space_used < no_large_search:
                    ...
                    print(f"start to use large search space for {large_space_used + 1} times")
...
def main(args):
    ...
    if dir_name:
        ppath = os.getcwd() + '/output/' + dir_name + '/'
    else:
        now = datetime.now()
        dir_name = now.strftime("%d_%m_%Y_%H_%M_%S")
        ppath = os.getcwd() + '/output/' + dir_name + '/'
    Path(ppath).mkdir(parents=True, exist_ok=True)
    n, d, s0, graph_type, sem_type = args.n, args.d, args.s0, args.graph_type, args.sem_type
    dims, no_large_search, size_small, size_large, learning_rate, num_iter, lambda1, lambda2, loss_type, opti, lr_decay = args.dims, args.no_large_search, args.size_small, args.size_large, args.learning_rate, args.num_iter, args.lambda1, args.lambda2, args.loss_type, args.opti, args.lr_decay
    seeds = args.seeds
    for seed in seeds:
        txt_name = ppath + f"{seed}_{n}_{d}_{s0}_{graph_type}_{sem_type}.txt"
        ...
        Topo_nonlinear = TOPO_Nonlinear(X = X, model = Topo_mlp,num_iter = num_iter,
                                    ...
                                    opti = opti, lr_decay = lr_decay,learning_rate = learning_rate,
                                    B_true = B_true)
        ...
        W_thres = threshold_W(W,threshold= 0.3)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dir_name', type=str, default='hyper_search')
    ...
    parser.add_argument('--no_large_search', type=int, default=1)
    parser.add_argument('--size_small', type=int, default=10)
    parser.add_argument('--size_large', type=int, default=30)
