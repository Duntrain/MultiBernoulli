...
from utils import best_shd

class TOPO_linear:
    def __init__(self, score, regress,verbose = False):
        super().__init__()
        self.score = score
    ...
    def fit(self, X, topo: list, no_large_search, size_small, size_large):
        ...
        self.vprint(f"current loss {loss}")
        print(best_shd(W_true = B_true,W =  self.W))
        ...
        while bool(idx_set):
            ...
            if np.any(loss > np.min(loss_collections)):
                ...
                self.vprint(f"find the better loss from {loss} -> {np.min(loss_collections)}")

            else:
                if large_space_used < no_large_search:
                    self.vprint(f"start to use large search space for {large_space_used+1} times")
                    ...
                    if np.any(loss > loss_collections):
                        ...
                        self.vprint("Using larger search space, but we cannot find better loss")
                        break


                else:
                    self.vprint("We reach the number of chances to search large space, it is {}".format(
                        no_large_search))
                    break
...
if __name__ == '__main__':
    ...
    def regress(X,y,C = 0.1):
        reg = LogisticRegression(multi_class='ovr', fit_intercept=True, penalty='none',
                                 solver='lbfgs',max_iter=1000)                         
        reg.fit(X = X, y = y)
        return reg.coef_

    def score(X,W,C = None):
        if C is None:
            lambda1 = 0
        else:
            lambda1 = 1/C
    ...
    from Topo_utils import find_topo    
    model = TOPO_linear(regress=regress, score=score,verbose = True)
    topo_init = list(np.random.permutation(range(d)))
    topo_true = list(find_topo(B_true))
    start = timer()
    W_est, _, _, _ = model.fit(X=X, topo=topo_true, no_large_search=no_large_search, size_small=size_small, size_large=size_large)
    end = timer()
    print(best_shd(W_true = B_true,W =  W_est))
